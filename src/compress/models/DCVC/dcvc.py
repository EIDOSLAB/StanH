...
class IntraNoAR(CompressionModel):
    def __init__(self, N=192, anchor_num=4, q_global = [0.5,1,1.2,1.5], lmbda_list = [0.0018,0.0036,0.009,0.025]):
        ...
        assert len(self.q_global) == self.anchor_num

        self._initialize_weights()


        print("VOGLIO IL NUMERO DI PARAMETRI!!!!!!")
        print(" TOTALE: ",sum(p.numel() for p in self.parameters()))
        print(" y_prior_fusion: ",sum(p.numel() for p in self.y_prior_fusion.parameters()))
    ...
    def forward(self, x, index = 0, q_scale=None):
        ...
        y_res, y_q, y_hat, scales_hat = self.forward_dual_prior( y, means, scales, q_step, self.y_spatial_prior)
